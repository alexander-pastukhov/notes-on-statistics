...
cor(Z)
```
Below is the plot of the original data. Note two highlighted points, here, values of x (positive for a red dot, negative for a lube one) are uncorrelated with values of y (roughly zero, which is why I have picked these two points). 
```{r echo=FALSE}
ggplot() + 
  ...
  geom_text(aes(x = c(0, 1) * 1.2, y = c(1, 0) * 1.2, label = c("\U00ee", "\U0135")), color="blue") +
  coord_equal() +
  xlab("x") +
  ylab("y") +
...
ggplot() + 
  ...
  geom_text(aes(x = L[, 2] * c(1.1, 1.25), y = L[, 1] * 1.1, label = c("\U00ee", "\U0135")), color="blue") +
  coord_equal() +
  xlab("x") +
  ylab("y") +
...
ggplot() +
  geom_segment(data=NULL, aes(x=Z[, 1], y=Z[, 2], xend=M[, 1], yend=M[, 2])) +
