...
## Another technical problem: Can we assume that responses correspond to real numbers?
The skewed residuals described above are a fundamental problem for parametric frequentist methods but is not critical if you use Bayesian or non-parametric bootstrapping/permutation linear models. Does this mean it is safe to use them? Probably not because when you use responses directly, you assume a direct correspondence between a response label (e.g., "4") and a real number $4$. If that is the case, you can assume that $(4 + 4) / 2$ is equal to $(3 + 5) / 2$ to $(2 + 6) / 2$ to $(1 + 7)/ 2$. However, what if this is _not_ the case, what if the cut-points (responses) do not correspond to the real numbers that you've picked? Then our basic arithmetic stops working! Take a look at the figure below where "real value" of responses is not an integer.
